...
def channel(pkt_loss_rate, svr_base_port, ch_base_port, cl_base_port, ch_number,
    ...
    q.put("CH{0:d} is ready.".format(ch_number))
...
if __name__ == '__main__':
    ...
    # for synchronization of processes for a server, a client, and channels
    q = multiprocessing.Queue()
    
    # start a server
    t_svr = multiprocessing.Process(target=server,
                             kwargs=dict(
    ...
    # start channels
    print(q.get())              # wait for the server
    for i in range(n_channels):
        t_ch = multiprocessing.Process(target=channel,
                                kwargs=dict(
                                    pkt_loss_rate=pkt_loss_rate,
                                    svr_base_port=svr_base_port,
                                    ch_base_port=ch_base_port,
    ...
    # start a client
    for i in range(n_channels):
        print(q.get())          # wait for the channels
    t_cl = multiprocessing.Process(target=client,
                            kwargs=dict(
                                n_channels=n_channels,
                                ch_base_port=ch_base_port,
                                cl_base_port=cl_base_port,
