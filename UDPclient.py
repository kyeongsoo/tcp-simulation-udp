import getopt
from socket import *
from sys import *
import random
from time import *
import select
import operator


serverName = '127.0.0.1'
serverPort = 12222
init = t = time()
cwnd = 1
num_of_pkts = 26
index = 0
timeout = 1
seq_array = []  # stores the sequence no for each alphabet

argv = argv[1:]
try:
    opts, args = getopt.getopt(argv, "N:p:t:s:")
except getopt.GetoptError:
    print('{0} -N <Number of pkts> -t <time out> -p <server_port_no> -s <Server_IP>'.format(__file__))
    exit(2)
for opt, arg in opts:
    if opt == '-h':
        print('{0} -i <IP_address> -t <Attachment_text> -p <Attachment_image> -r <Recipient> -s <Sender> -d <data>'.format(__file__))
        exit()
    elif opt in "-N":
        num_of_pkts = int(arg)
    elif opt in "-t":
          timeout = int(arg)
    elif opt in "-p":
        serverPort = int(arg)
    elif opt in "-s":
        serverName = arg

                
def pkt_send(pkt_snd: object) -> object:
    """ Send a packet to the server """
    print("SENT: "+pkt_snd)
    clientSocket.sendto(pkt_snd.encode(), server)


def pkt_receive():
    """ Receive a packet from the server """
    bytes, address = clientSocket.recvfrom(300)
    return bytes.decode(), address


def pkt_generate(ch, index):    # for generating pkt
    if alpha_use[index] == 0:
        strlen = random.randint(1,100)
        alpha_use[index] = strlen
    else:
        strlen = alpha_use[index]
    pkt_snd = [ch for x in range(strlen)]  # array of characters
    pkt_snd = "".join(pkt_snd)  # concatenating all characters to generate string
    index += 1
    return pkt_snd, index, strlen


def pkt_time():  # to display time for each pkt sent and received
    t = time()
    return t-init


def lost_index(seq_array: object, rcvd_ack: object) -> object:
    """ Find the corresponding alphabet using sequence number """
    for x in range(0, num_of_pkts):
        if rcvd_ack == seq_array[x]:
            return x


alpha_use = [0 for x in range(num_of_pkts)]  # keeps track of no. of alphabets
server = serverName, serverPort
clientSocket = socket(AF_INET, SOCK_DGRAM)

seq = random.randint(1, 1000)
ack = 1
cnt = 1
pkt_snd = "seq="+str(seq)+";ack=1;S=1;len=0"
pkt_send(pkt_snd)
print("SENT TIME: " + str(pkt_time()))

pkt_rcv, serverAddress = pkt_receive()
print("RCVD: "+pkt_rcv)
print("RCVD TIME: " + str(pkt_time()))

pkt_rcv = pkt_rcv.split(';')
if pkt_rcv[2] == "SA=1":
    ack = int(pkt_rcv[0].partition('=')[2])
    rcvd_ack = int(pkt_rcv[1].partition('=')[2])
    prev_rcvd_ack = rcvd_ack
    seq = seq+1
    pkt_snd = "seq=" + str(seq) + ";ack=" + str(ack) + ";A=1;len=0"
    pkt_send(pkt_snd)
    print("CONNECTION GENERATED: " + str(pkt_time()))
    seq = seq+1

    ch = chr(operator.mod(index, 26)+65)
    pkt_snd, index, strlen = pkt_generate(ch, index)
    pkt_send("seq=" + str(seq) + ";ack=" + str(ack) + ";A=1;len=" + str(strlen) + ";data=" + pkt_snd)
    print("PKT SENT TIME: " + str(pkt_time()))
    seq_array.append(seq)
    seq = seq + strlen
    seq_array.append(seq)

    flag = True
    while cwnd:
        temp = cwnd
        while temp:
            ready = select.select([clientSocket], [], [], timeout)
            print("\n\n\n")
            if ready[0]:
                pkt_rcv, serverAddress = pkt_receive()
                print("RCVD: "+pkt_rcv)
                print("PKT RCVD TIME: " + str(pkt_time()))
                pkt_rcv = pkt_rcv.split(';')
                ack = int(pkt_rcv[0].partition('=')[2])
                rcvd_ack = int(pkt_rcv[1].partition('=')[2])
                temp -= 1
                if rcvd_ack == prev_rcvd_ack and flag:
                    cnt = cnt+1
                    if cnt == 3:
                        cwnd = cwnd/2
                        temp = cwnd  # for triple duplicate acks, cwnd is halved
                        print("\n\n*********Received triple duplicate ACKs*********\n\n")
                        cnt = 1
                        flag = False
                        index = lost_index(seq_array, rcvd_ack)
                        seq = seq_array[index]
                elif rcvd_ack != prev_rcvd_ack:
                    flag = True
                prev_rcvd_ack = rcvd_ack

                if index == num_of_pkts:
                    print("Packets delivered")  # do nothing, wait for server to send all acks
                else:
                    ch = chr(operator.mod(index, 26)+65)
                    pkt_snd, index, strlen = pkt_generate(ch, index)
                    pkt_send("seq=" + str(seq) + ";ack=" + str(ack) + ";A=1;len=" + str(strlen) + ";data=" + pkt_snd)
                    print("PKT SENT TIME: " + str(pkt_time()))
                    seq = seq + strlen
                    if seq not in seq_array:
                        seq_array.append(seq)

                if index == num_of_pkts:
                    # print "\nCONNECTION IS CLOSING!!" # ############## Condition start from here
                    pkt_snd = "seq="+str(seq)+";ack="+str(ack)+";F=1;len=0"
                    pkt_send(pkt_snd)
                    seq = seq+1
                    pkt_rcv, serverAddress = pkt_receive()
                    print("RCVD: "+pkt_rcv)
                    pkt_rcv = pkt_rcv.split(';')
                    # if seq == int(pkt_rcv[1].partition('=')[2]):
                    if pkt_rcv[2] == "F=1":
                        pkt_rcv, serverAddress = pkt_receive()
                        print("RCVD: "+pkt_rcv)
                        pkt_rcv = pkt_rcv.split(';')
                        if pkt_rcv[2] == "F=1":
                            pkt_snd = "seq="+pkt_rcv[1].partition('=')[2]+";ack="+str(int(pkt_rcv[0].partition('=')[2]))+";len=0"
                            pkt_send(pkt_snd)
                            clientSocket.close()
                            exit()
                else:
                    ch = chr(operator.mod(index, 26)+65)
                    pkt_snd, index, strlen = pkt_generate(ch, index)
                    pkt_send("seq=" + str(seq) + ";ack=" + str(ack) + ";A=1;len=" + str(strlen) + ";data=" + pkt_snd)
                    print("PKT SENT TIME: " + str(pkt_time()))
                    seq = seq + strlen
                    if seq not in seq_array:
                        seq_array.append(seq)
            else:
                index = lost_index(seq_array, rcvd_ack)
                ch = chr(operator.mod(index,26)+65)
                print("ACK not received for", ch, "at time ", pkt_time())
                pkt_snd, index, strlen = pkt_generate(ch, index)
                seq = seq_array[index]
                pkt_send("seq=" + str(seq) + ";ack=" + str(ack) + ";A=1;len=" + str(strlen) + ";data=" + pkt_snd)

        cwnd = 2*cwnd
